...
async function sendOwnerEmail(data) {
  try {
    await sendGridEmail(
      ...
      'New Event Registration — ' + (data.eventName || 'Event'),
    ...
    console.log('Owner event email sent via SendGrid');
  } catch (e) {
...
async function sendCustomerEmail(data) {
  try {
    await sendGridEmail(
      data.email,
      'Registration Confirmed — ' + (data.eventName || 'The Quarry Event'),
      '<div style="font-family:Arial,sans-serif;max-width:600px">' +
      '<div style="background:#1A0E08;padding:24px;text-align:center">' +
      '<h1 style="color:#B8933A;margin:0">The Quarry</h1>' +
      '<p style="color:#F5F0E8;font-size:0.8rem;letter-spacing:0.15em;margin:4px 0 0">NEW MELLE, MISSOURI</p></div>' +
    ...
    console.log('Customer event email sent via SendGrid');
  } catch (e) {
