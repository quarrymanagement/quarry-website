// ============================================================================
// list-pavilion-bookings.js
//
// Admin-only: returns all pavilion bookings within a date range. Blob
// storage (pavilion-bookings/{date}) is the sole source of truth here --
// unlike golf, which also reconciles against Stripe's own session list,
// every pavilion booking (Square-paid or admin-blocked) is written there
// directly by pavilion-checkout.js / square-webhook.js, so there's nothing
// else to reconcile against.
//
// GET /.netlify/functions/list-pavilion-bookings?token=...&startDate=...&endDate=...
...
exports.handler = async (event) => {
  ...
  while (cursor <= endDate && dayCount < 365) {
    try {
      const data = await readBlob('pavilion-bookings/' + cursor);
      if (data) {
